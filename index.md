---
layout: default
hide_header: true
extra_head: |
  <script>document.documentElement.classList.add('landing-page');</script>
  <link rel="stylesheet" href="/assets/css/landing.css">
---

<div class="landing-hero">
  <div class="landing-hero-content">
    <h1 class="landing-name">Hyogon<br>Ryu.</h1>
    <p class="landing-role">Applied AI Research Engineer</p>
    <p class="landing-affiliation">Krafton AI</p>
  </div>
  <div class="landing-scroll-hint">
    <div class="landing-scroll-line"></div>
  </div>
</div>

<div class="landing-nav-section">
  <a class="landing-nav-item" href="/about">
    <span class="landing-nav-label">About</span>
    <span class="landing-nav-desc">Research, publications, awards, and career.</span>
    <span class="landing-nav-arrow">&rarr;</span>
  </a>
  <a class="landing-nav-item" href="/playground">
    <span class="landing-nav-label">Playground</span>
    <span class="landing-nav-desc">A tiny open world where AI NPCs live.</span>
    <span class="landing-nav-arrow">&rarr;</span>
  </a>
  <a class="landing-nav-item" href="/posts">
    <span class="landing-nav-label">Posts</span>
    <span class="landing-nav-desc">Dev logs and tech reports.</span>
    <span class="landing-nav-arrow">&rarr;</span>
  </a>
</div>
